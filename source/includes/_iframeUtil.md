...
To respond only to a specific Widget instance then check the value of the `e.frameId` parameter.


```javascript
// Respond only to the 'getDataResult' event from the 'wiwo-bojumo' iframe:
var _wiwo = _wiwo || [];
_wiwo.push(['on', 'wiwo.dido.getDataResult', function(e, data){
	if (e.frameId != 'wiwo-bojumo'){
		return;
	}
...
`iframeUtil.on(eventName, handler)`

`_wiwo.push(['on', eventName, handler])`
...
// FrameEvent structure:
{
	...
	"frameId": "wiwo-bojumo",	// The ID of the element that raised the event.
...
Parameter | Type       | Description
--------- | ---------  | -----------
e         | FrameEvent | Information about the iframe (i.e. Widget) that raised this event.
...
```javascript
// Listen for the `getDataResult` event from any Widget:
var _wiwo = _wiwo || [];
...
```javascript
// Request data from the 'wiwo-bojumo' Widget iframe.
...
```javascript
// Set new data on the 'wiwo-bojumo' Widget.
